...
Documentation is auto-generated by Swagger. Can be accessed at [localhost:8000/docs](http://localhost:8000/docs) by default when running the project locally.

Credentials for the first superuser that is auto-created are defined by environment variables. Refer to the [`.env-sample` file](https://github.com/vinivosh/planne-back-end-challenge/blob/main/.env-sample) for more information.


## Project architecture

This project is a monorepo actually containing two modular applications.

- **FruitFULL** is the main back-end server, implementing RESTful APIs with FastAPI, letting us login as users or super-users to view, create, update or delete fruits and buckets. It has OAuth2 authentication.
- **Planne SDK** is a python package containing all data models and use cases for interacting with the DB while enforcing business logic. It is used extensively by *FruitFULL*. Using this helps keep code repetition to a mininum, and is really useful if we were to extend the ecosystem with other applications or micro-services.
...
Create a valid `.env` file at the root of the repository folder, with all required variables defined. Refer to the [`.env-sample`](https://github.com/vinivosh/planne-back-end-challenge/blob/main/.env-sample) file for more information. The same must be done for each one of the projects in this repo, [fruit-full](https://github.com/vinivosh/planne-back-end-challenge/blob/main/fruit-full/.env-sample) and [planne-sdk](https://github.com/vinivosh/planne-back-end-challenge/blob/main/planne-sdk/.env-sample).
